...
template <class Cell>
HierarchicalArray2D<Cell>::HierarchicalArray2D(int xsize, int ysize,
                                               int patchMagnitude)
...
HierarchicalArray2D<Cell>::HierarchicalArray2D(const HierarchicalArray2D& hg)
    : Array2D<autoptr<Array2D<Cell> > >::Array2D(
          ...
          (hg.m_ysize >>
           hg.m_patchMagnitude))  // added by cyrill: if you have a resize
...
template <class Cell>
HierarchicalArray2D<Cell>& HierarchicalArray2D<Cell>::operator=(
...
void HierarchicalArray2D<Cell>::allocActiveArea() {
  for (PointSet::const_iterator it = m_activeArea.begin();
       it != m_activeArea.end(); it++) {
...
Cell& HierarchicalArray2D<Cell>::cell(int x, int y) {
  ...
  autoptr<Array2D<Cell> >& ptr = this->m_cells[c.x][c.y];
  return (*ptr).cell(
      IntPoint(x - (c.x << m_patchMagnitude), y - (c.y << m_patchMagnitude)));
}
