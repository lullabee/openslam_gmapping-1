...
GridSlamProcessor::TNode::TNode(const OrientedPoint& p, double w, TNode* n,
                                unsigned int c) {
  pose = p;
  weight = w;
  childs = c;
...
GridSlamProcessor::TNodeVector GridSlamProcessor::getTrajectories() const {
  ...
  for (ParticleVector::const_iterator it = m_particles.begin();
    ...
    if (newnode->parent) {
      ...
      if (!newnode->parent->flag) {
        // cerr << __PRETTY_FUNCTION__ << ": node " << newnode->parent << "
        // flag=" << newnode->parent->flag<< endl;
  ...
  // cerr << __PRETTY_FUNCTION__ << ": border.size(INITIAL)=" << border.size()
  // << endl; cerr << __PRETTY_FUNCTION__ << ": parentCache.size()=" <<
  // parentCache.size() << endl;
  while (!border.empty()) {
    // cerr << __PRETTY_FUNCTION__ << ": border.size(PREPROCESS)=" <<
    // border.size() << endl; cerr << __PRETTY_FUNCTION__ << ":
    // parentCache.size(PREPROCESS)=" << parentCache.size() << endl;
    ...
    pair<TNodeMultimap::iterator, TNodeMultimap::iterator> p =
        parentCache.equal_range(node);
    double childs = 0;
    ...
    // cerr << __PRETTY_FUNCTION__ << ": parentCache.size(POSTERASE)=" <<
    // parentCache.size() << endl;
...
// void GridSlamProcessor::integrateScanSequence(GridSlamProcessor::TNode*
// node){
// 	//reverse the list
// 	TNode* aux=node;
// 	TNode* reversed=0;
...
// 			it->pose.theta=atan2(sin(it->pose.theta),
// cos(it->pose.theta));

// 			//register the scan
// 			m_matcher.invalidateActiveArea();
// 			m_matcher.computeActiveArea(it->map, it->pose,
// plainReading); 			it->weight+=dw; 			it->weightSum+=dw;

// 			// this should not work, since it->weight is not the correct
// weight!
// 			//			it->node=new TNode(it->pose, it->weight,
// it->node); 			it->node=new TNode(it->pose, 0.0, it->node);
// 			//update the weight
// 		}
...
double GridSlamProcessor::propagateWeights() {
  ...
  if (fabs(aw - 1.0) > 0.0001 || fabs(lastNodeWeight - 1.0) > 0.0001) {
    cerr << "ERROR: ";
    cerr << "root->accWeight=" << lastNodeWeight
