...
GridSlamProcessor::GridSlamProcessor() : m_infoStream(cout) {
  ...
  m_minimumScore = 0.;
  m_outputStream.open("/tmp/loggmapping.log", std::ofstream::out);
}
...
void GridSlamProcessor::setSensorMap(const SensorMap& smap) {
  ...
  FIXME For now detect the readings of only the front laser, and assume its pose
  is in the center of the robot
  */
