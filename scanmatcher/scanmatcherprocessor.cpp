...
ScanMatcherProcessor::ScanMatcherProcessor(const ScanMatcherMap& m)
    : m_map(m.getCenter(), m.getWorldSizeX(), m.getWorldSizeY(),
            m.getResolution()),
      m_pose(0, 0, 0) {
  m_regScore = 300;
...
ScanMatcherProcessor::ScanMatcherProcessor(double xmin, double ymin,
                                           double xmax, double ymax,
                                           double delta, double patchdelta,
                                           bool use_icp)
    : m_map(Point((xmax + xmin) * .5, (ymax + ymin) * .5), xmax - xmin,
            ymax - ymin, patchdelta),
      m_pose(0, 0, 0) {
  m_regScore = 300;
  m_critScore = .5 * m_regScore;
  m_maxMove = 1;
...
void ScanMatcherProcessor::processScan(const RangeReading& reading) {
  ...
  // cout << "rel-move x="<< move.x <<  " y=" << move.y << " theta=" <<
  // move.theta << endl;
  ...
  cout << "abs-move x=" << dPose.x << " y=" << dPose.y
       << " theta=" << dPose.theta << endl;
#endif
  m_pose = m_pose + dPose;
  m_pose.theta = atan2(sin(m_pose.theta), cos(m_pose.theta));

#ifdef SCANMATHCERPROCESSOR_DEBUG
  cout << "StartPose: x=" << m_pose.x << " y=" << m_pose.y
       << " theta=" << m_pose.theta << endl;
#endif
...
gsl_matrix_set(m,0,0,cov.xx); gsl_matrix_set(m,0,1,cov.xy);
gsl_matrix_set(m,0,2,cov.xt); gsl_matrix_set(m,1,0,cov.xy);
gsl_matrix_set(m,1,1,cov.yy); gsl_matrix_set(m,1,2,cov.yt);
gsl_matrix_set(m,2,0,cov.xt); gsl_matrix_set(m,2,1,cov.yt);
gsl_matrix_set(m,2,2,cov.tt); gsl_matrix* evec=gsl_matrix_alloc(3,3);
...
#ifdef SCANMATHCERPROCESSOR_DEBUG
      // cout << "evals=" << gsl_vector_get(eval, 0) <<  " " <<
      // gsl_vector_get(eval, 1)<< " " << gsl_vector_get(eval, 2)<<endl;
...
#ifdef SCANMATHCERPROCESSOR_DEBUG
  cout << " FinalPose: x=" << newPose.x << " y=" << newPose.y
       << " theta=" << newPose.theta << endl;
...
void ScanMatcherProcessor::setMatchingParameters(double urange, double range,
                                                 double sigma, int kernsize,
...
void ScanMatcherProcessor::setRegistrationParameters(double regScore,
                                                     double critScore) {
  m_regScore = regScore;
