...
const double* readings){ if (m_activeAreaComputed) return;
        ...
        for (const double* r=readings; r<readings+m_laserBeams; r++, angle++)
                if (m_generateMap){
                        ...
                        Point
phit=lp+Point(d*cos(lp.theta+*angle),d*sin(lp.theta+*angle)); IntPoint
p1=map.world2map(phit);

                        d+=map.getDelta();
                        //Point
phit2=lp+Point(d*cos(lp.theta+*angle),d*sin(lp.theta+*angle));
...
void ScanMatcher::computeActiveArea(ScanMatcherMap& map, const OrientedPoint& p,
  ...
  if (!map.isInside(min) || !map.isInside(max)) {
    ...
    // cerr << "CURRENT MAP " << lmin.x << " " << lmin.y << " " << lmax.x << " "
    // << lmax.y << endl; cerr << "BOUNDARY OVERRIDE " << min.x << " " << min.y
    // << " " << max.x << " " << max.y << endl;
  ...
  for (const double* r = readings + m_initialBeamsSkip;
    ...
    if (m_generateMap) {
      ...
      Point phit =
          lp + Point(d * cos(lp.theta + *angle), d * sin(lp.theta + *angle));
      IntPoint p0 = map.world2map(lp);
      IntPoint p1 = map.world2map(phit);
  ...
  /*
     ...
     it=activeArea.begin(); it!= activeArea.end(); it++){ cerr << "(" << it->x
     <<"," << it->y << ") ";
          }
          cerr << endl;
  */
...
const double* readings){ if (!m_activeAreaComputed) computeActiveArea(map, p,
readings);

        //this operation replicates the cells that will be changed in the
registration operation map.storage().allocActiveArea();
...
p1=map.world2map(phit);
                        ...
                        for (int i=0; i<line.num_points-1; i++){
                                IntPoint
ci=map.storage().patchIndexes(line.points[i]); if
(map.storage().getActiveArea().find(ci)==map.storage().getActiveArea().end())
                                        cerr << "BIG ERROR" <<endl;
                                map.cell(line.points[i]).update(false,
Point(0,0));
                        }
                        if (d<=m_usableRange){
...
double ScanMatcher::icpOptimize(OrientedPoint& pnew, const ScanMatcherMap& map,
  ...
  do {
    ...
    // endl; cerr << "pret=" << pnew.x << " " <<pnew.y << " " << pnew.theta <<
    // endl;
    start = pnew;
    iterations++;
...
double ScanMatcher::optimize(OrientedPoint& _mean,
  ...
  do {
    ...
    do {
      ...
      // update the score
      count++;
      likelihoodAndScore(localScore, localLikelihood, map, localPose, readings);
      if (localScore > currentScore) {
        currentScore = localScore;
        bestLocalPose = localPose;
      }
  ...
  // cout << "delta.x=" << delta.x << " delta.y=" << delta.y << " delta.theta="
  // << delta.theta << endl;
  ...
  cov.xx /= lacc, cov.xy /= lacc, cov.xt /= lacc, cov.yy /= lacc,
      cov.yt /= lacc, cov.tt /= lacc;
...
double ScanMatcher::likelihood(double& _lmax, OrientedPoint& _mean,
                               CovarianceMatrix& _cov,
                               const ScanMatcherMap& map,
                               const OrientedPoint& p, const double* readings) {
  ScoredMoveList moveList;

  for (double xx = -m_llsamplerange; xx <= m_llsamplerange;
       xx += m_llsamplestep)
    for (double yy = -m_llsamplerange; yy <= m_llsamplerange;
         yy += m_llsamplestep)
      for (double tt = -m_lasamplerange; tt <= m_lasamplerange;
           tt += m_lasamplestep) {
        OrientedPoint rp = p;
        rp.x += xx;
        rp.y += yy;
  ...
  // OrientedPoint delta=mean-currentPose;
  // cout << "delta.x=" << delta.x << " delta.y=" << delta.y << " delta.theta="
  // << delta.theta << endl; normalize the likelihood
  double lmax = -1e9;
  double lcum = 0;
  for (ScoredMoveList::const_iterator it = moveList.begin();
       it != moveList.end(); it++) {
  ...
  cov.xx /= lcum, cov.xy /= lcum, cov.xt /= lcum, cov.yy /= lcum,
      cov.yt /= lcum, cov.tt /= lcum;
...
double ScanMatcher::likelihood(double& _lmax, OrientedPoint& _mean,
                               CovarianceMatrix& _cov,
                               const ScanMatcherMap& map,
                               const OrientedPoint& p, Gaussian3& odometry,
                               const double* readings, double gain) {
  ScoredMoveList moveList;
...
void ScanMatcher::setMatchingParameters(double urange, double range,
                                        double sigma, int kernsize, double lopt,
                                        double aopt, int iterations,
