...
double icpStep(OrientedPoint& retval, const PointPairContainer& container) {
  ...
  for (ContainerIterator it = container.begin(); it != container.end(); it++) {
    PointPair mf =
        std::make_pair(it->first - mean.first, it->second - mean.second);
...
template <typename PointPairContainer>
double icpNonlinearStep(OrientedPoint& retval,
  ...
  for (ContainerIterator it = container.begin(); it != container.end(); it++) {
    PointPair mf =
        std::make_pair(it->first - mean.first, it->second - mean.second);
    double dalpha =
        atan2(mf.second.y, mf.second.x) - atan2(mf.first.y, mf.first.x);
