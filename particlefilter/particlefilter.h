...
double toNormalForm(OutputIterator& out, const Iterator& begin,
                    const Iterator& end) {
...
void toLogForm(OutputIterator& out, const Iterator& begin, const Iterator& end,
               Numeric lmax) {
  // determine the maximum
  for (Iterator it = begin; it != end; it++) {
...
void resample(std::vector<int>& indexes, const WeightVector& weights,
              unsigned int nparticles = 0) {
  double cweight = 0;

  // compute the cumulative weights
  unsigned int n = 0;
  for (typename WeightVector::const_iterator it = weights.begin();
       it != weights.end(); ++it) {
    cweight += (double)*it;
    n++;
  }
...
void repeatIndexes(Vector& dest, const std::vector<int>& indexes,
...
struct uniform_resampler {
  std::vector<unsigned int> resampleIndexes(
...
template <class Particle, class Numeric>
Numeric uniform_resampler<Particle, Numeric>::neff(
...
template <class Particle, class EvolutionModel>
void evolver<Particle, EvolutionModel>::evolve(
    std::vector<Particle>& particles) {
  for (typename std::vector<Particle>::iterator it = particles.begin();
       it != particles.end(); ++it) {
...
void evolver<Particle, EvolutionModel>::evolve(
  ...
  dest.clear();
  for (typename std::vector<Particle>::const_iterator it = src.begin();
       it != src.end(); ++it)
    dest.push_back(evolutionModel.evolve(*it));
}

template <class Particle, class Numeric, class QualificationModel,
          class EvolutionModel, class LikelyhoodModel>
...
void auxiliary_evolver<Particle, Numeric, QualificationModel, EvolutionModel,
                       LikelyhoodModel>::evolve(std::vector<Particle>&
                                                    particles) {
  ...
  for (typename std::vector<Particle>::const_iterator it = particles.begin();
    ...
    observationWeights[i] =
        likelyhoodModel.likelyhood(qualificationModel.evolve(*it));
  }
  uniform_resampler<Numeric, Numeric> resampler;
  std::vector<unsigned int> indexes(
      resampler.resampleIndexes(observationWeights));
  for (typename std::vector<unsigned int>::const_iterator it = indexes.begin();
       it != indexes.end(); it++) {
    Particle& particle = particles[*it];
    particle = evolutionModel.evolve(particle);
    particle.setWeight(likelyhoodModel.likelyhood(particle) /
                       observationWeights[*it]);
  }
}
...
void auxiliary_evolver<Particle, Numeric, QualificationModel, EvolutionModel,
  ...
  for (typename std::vector<unsigned int>::const_iterator it = indexes.begin();
    ...
    dest.back().weight *=
        likelyhoodModel.likelyhood(particle) / observationWeights[*it];
  }
}
// END legacy
